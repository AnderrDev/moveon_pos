...
| PLAN-67 | ✅ Hecho — Cableado hexagonal: `auth` + `audit` + limpieza de excepciones del linter | P2 | `audit`: los 6 consumidores que inyectaban la implementación concreta (`products`, `cash-register`, `sales`, `pos/pos-sale.service.ts`, `inventory`, y la propia `auditoria.page.ts`) pasan a importar `AuditLogRepository` desde `domain/repositories/` en vez de `data/repositories/` — mismo nombre de clase, solo cambia la ruta del import (el contrato ya existía desde PLAN-62, esto era el flip de DI pendiente). `auth`: auditada, sin cambios de código — no tiene patrón repository por diseño (usa `core/auth/session.service.ts`, transversal a toda la app); sus 3 páginas/presenters/forms ya estaban correctamente co-ubicados desde PLAN-61 y no importan `data/` de ninguna feature ni cruzan fronteras. `CABLED_FEATURES` del linter pasa a `['audit', 'auth', 'customers', 'products']` — `auth` se agrega sin que cambie ningún import real, solo para que la regla la proteja hacia adelante si algún día gana un `data/`. Excepciones de `PRESENTATION_BOUNDARY_EXCEPTIONS` (5, de PLAN-63): **ninguna se retiró** — las 5 dependen de `cash-register`, `settings`, `expenses`, `pos` o `products`, todas pendientes de PLAN-65/66 (en progreso en paralelo en otros worktrees), no de `audit`/`auth`. La fila original de este PLAN asumía que se podían retirar TODAS; eso solo es cierto una vez PLAN-65 y 66 cierren. Verificado: typecheck+build+lint+511 tests (sin cambio de conteo — no se tocó lógica ni tests, solo rutas de import y config) |
...
| PLAN-70 | ✅ Hecho (local) — Consulta pública del progreso MOVE ON Club en el catálogo | P2 | Según **ADR 0016**: RPC `storefront_club_progress(p_celular)` SECURITY DEFINER con grant a `anon` — proyección mínima cerrada (primer nombre, sellos X/N, recompensas disponibles con valor tope y vencimiento más próximo; nunca documento/teléfono/email/historial), solo clientes activos con `autoriza_fidelizacion = true`, sin oráculo de existencia (respuesta vacía idéntica para no-encontrado/inactivo/sin-autorización); normalización de celular igual a `phone-co.ts`. UI en `apps/landing-web`: sección "Mi tarjeta MOVE ON Club" con input de celular → tarjeta de sellos visual (casillas troqueladas, identidad negro + amarillo #F9D128), mensaje genérico único de no-encontrado, sin persistir el celular en el navegador. Tests: RPC (encontrado/no autoriza/inactivo/formatos de celular) + dominio de la proyección. Verificado en local antes de aplicar a prod |

| PLAN-71 | ✅ Hecho (2026-08-02, aplicado al remoto) — Opciones de producto en la venta: tipo de proteína del batido | P1 | Según **ADR 0017**: tabla `product_options` (grupo, nombre, `precio_extra`, componente de inventario, default) administrable desde el formulario de producto; `sale_items` gana `option_id`/`option_nombre`/`option_extra` (snapshots); `create_sale_atomic` calcula el precio efectivo como `precio_venta + precio_extra` en el servidor y valida que la opción pertenezca al producto; `tg_consume_sale_components` descuenta el componente de la opción (sachet). POS: diálogo de selección al agregar un batido, la opción es parte de la identidad de la línea (`key = productId:optionId`), visible en ticket e historial. Seed: CH+ default $0 sin componente, Bipro +$2.000 con 1 sachet Bipro, Best Whey +$1.000 con 1 sachet Best Whey, sobre `BATIDO EN AGUA` y `BATIDO EN LECHE` |

| PLAN-72 | Corregir movimientos del turno de caja (RN-C16) | P1 | `correct_cash_movement_atomic` (monto + concepto, espejo de `correct_cash_session_opening_atomic`): gate de caja compartida en vez de admin-only, solo con la sesión `open` y sobre movimientos `active`, motivo de corrección de mínimo 10 caracteres, no-ops rechazados, auditado en `audit_logs` con valores viejo y nuevo. El `tipo` no se corrige (invierte el signo del cuadre): para eso sigue la anulación (RN-C12). UI: botón "Corregir" en cada movimiento del turno en `/caja`, visible para cualquier usuario activo — "Anular" sigue siendo admin-only |

| PLAN-73 | ✅ Hecho (2026-08-03, verificado en local) — Combos y promociones como producto | P1 | Según **ADR 0018**: nuevo valor `'combo'` en el enum `product_type` (migración aparte, Postgres no deja usar un valor de enum en la misma transacción que lo agrega). Un combo es un producto con `precio_venta` fijo propio y UNA línea en el ticket, que al venderse descuenta los productos incluidos vía `product_components` — sin tabla nueva. Los guardas `tipo <> 'prepared'` pasan a `not in ('prepared','combo')` en `create_sale_atomic` (validación de stock y `sale_exit`), `tg_consume_sale_components`, `void_sale_atomic`, `create_product_with_initial_stock` y la vista `storefront_productos_publicos` (los combos no salen al catálogo público). POS: disponibilidad derivada `min(floor(stock/cantidad))` que topa la cantidad del carrito; el servidor mantiene "advertir, no bloquear". Formulario: tipo "Combo", sección "Productos incluidos" (solo productos con stock propio, sin anidamiento) y resumen en vivo de suma normal / precio / ahorro. Sin vigencia: `is_active`. **Además cierra la deuda de ADR 0017**: `void_sale_atomic` ahora devuelve al stock los componentes fijos y el componente de la opción al anular |

| PLAN-74 | ✅ Hecho (2026-08-04, verificado en local) — Costo capturado en la línea de venta | P1 | Según **ADR 0019**: `sale_items.unit_cost` llenado por `create_sale_atomic` en el momento de la venta, para que el margen de un período pasado deje de cambiar al actualizar un costo del catálogo. `simple`/`ingredient`/`prepared` toman `productos.costo`; un `combo` suma el costo de sus productos incluidos (su lista ES la lista completa de materiales, la de un batido no). Nulo = costo desconocido: la línea, y con ella el producto entero, se excluye de la utilidad — nunca se asume 0. Corrige de una vez los tres consumidores del costo actual: reporte de productos, utilidad neta de Finanzas y el COGS de `get_reinvestment_fund_totals` (que además deja de unir con `productos`). El reporte diario ya no carga el catálogo completo. Backfill de las líneas previas con el costo del catálogo de ese día, documentado como aproximación. Cubre el cálculo de utilidad, que no tenía ningún test |
