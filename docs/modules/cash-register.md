...
- RN-C15: una vez cerrada una sesión, sus movimientos y ventas dejaban de ser accesibles desde la UI. "Turnos anteriores" (`mo-closed-sessions-list`, solo `admin` — `canViewClosedSessions`) lista las sesiones `closed` de la tienda (hasta 30, más recientes primero) con su cuadre (esperado/real/diferencia); al expandir una fila carga de forma perezosa sus movimientos y ventas con el mismo detalle que tenía mientras estaba abierta.
