...
6. Productos vendidos con costo, utilidad y margen (`ReportsService.getDailyReport` → `productSales` / `utilidadTotal`). Usa el costo **capturado en la venta** (`sale_items.unit_cost`, ADR 0019), no el actual del catálogo: el margen de un período pasado ya no cambia cuando se actualiza un costo. Lo resuelve `create_sale_atomic` — `simple`/`ingredient`/`prepared` toman `productos.costo`; un `combo` suma el costo de sus productos incluidos. Si a una línea le falta el costo queda en `null` y el producto entero reporta utilidad desconocida (se muestra "—" en UI/Excel) y se excluye de `utilidadTotal`, nunca se asume costo 0. El costo unitario que se muestra es el **promedio ponderado** del período, porque el costo pudo cambiar entre ventas. Los batidos (`prepared`) siguen sin costo por decisión de ADR 0019 §2.2: la proteína base sale del tarro y no se mide por batido, así que costearlos con sus componentes inflaría el margen. **Las ventas anteriores al 2026-08-04 fueron rellenadas con el costo del catálogo de ese día y siguen siendo aproximaciones.**
