...
- **Fix (en dos pasos, con un diagnóstico intermedio equivocado):** usuario configuró "Package directory" = `apps/landing-web` en el dashboard. Con eso, Netlify empezó a leer `apps/landing-web/netlify.toml` — pero falló dos veces por rutas: `publish = "../../dist/landing-web/browser"` dio `must be inside the repository root directory`, y `publish = "dist/browser"` dio `Deploy directory 'dist/browser' does not exist`. El log del segundo fallo (leído desde el dashboard, la API no expone logs de build) mostró la causa real: **`publish` se resuelve relativo a la raíz del repo aunque el `netlify.toml` viva en el package directory** (`publish: /opt/build/repo/dist/browser`). El `../../` del primer intento escapaba de la raíz del repo, no del package directory como se creyó al inicio. Ruta final correcta: `publish = "apps/landing-web/dist/browser"`.
- Durante ese debugging se movió el `outputPath` de `landing-web` en `angular.json` de `dist/landing-web` a `apps/landing-web/dist`; se mantiene porque deja el artefacto autocontenido junto a la app.
