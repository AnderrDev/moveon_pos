...
- Botón `Abrir caja` en POS: `pnpm exec tsc --noEmit` pasó; `pnpm exec ngc -p apps/pos-angular/tsconfig.app.json` pasó; `pnpm test` pasó (52 archivos / 446 tests). En una primera pasada, `pnpm lint` fallaba por issues existentes fuera del botón; se corrigieron en la preparación para subir.
- Preparación para subir: se corrigieron los issues de lint existentes en POS (`auditoria.page.ts`, labels de productos/reportes, comparación estricta en productos y `any` de componentes). `pnpm lint`, `pnpm lint:landing`, `pnpm exec tsc --noEmit`, `pnpm exec ngc -p apps/pos-angular/tsconfig.app.json` y `pnpm test` pasan. `pnpm build` y `pnpm build:landing` siguen abortando con código 134 justo al iniciar `ng build`.
