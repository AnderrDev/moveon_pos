...
describe('formatTime', () => {
  it('formatea hora y minutos por defecto', () => {
    expect(formatTime(new Date(2026, 3, 27, 15, 4, 5))).toMatch(/03:04/)
  })

  it('permite incluir segundos', () => {
    expect(formatTime(new Date(2026, 3, 27, 15, 4, 5), { second: '2-digit' })).toMatch(/03:04:05/)
  })

  it('acepta fechas como string', () => {
    expect(formatTime('2026-04-27T15:04:05')).toMatch(/03:04/)
  })
})
