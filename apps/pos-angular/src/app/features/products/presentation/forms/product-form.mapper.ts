...
import type { ProductFormValue } from '@angular-app/features/products/presentation/forms/product-form.factory'
...
export const productFormMapper = {
  ...
  toFormValue(product?: Product | null): ProductFormValue {
    return {
      ...
      paraQueSirve: product?.paraQueSirve ?? '',
      recomendadoPara: product?.recomendadoPara ?? '',
  ...
  toCreatePayload(value: ProductFormValue, tiendaId: string): CreateProductDto {
    return {
      ...
      proveedor:    value.proveedor?.trim() || null,
      paraQueSirve: value.paraQueSirve?.trim() || null,
      recomendadoPara: value.recomendadoPara?.trim() || null,
