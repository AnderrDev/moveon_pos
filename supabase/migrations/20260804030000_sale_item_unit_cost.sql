-- =====================================================
-- Costo de la venta capturado en la línea (PLAN-74 / ADR 0019)
-- =====================================================
-- Hasta hoy la utilidad se calculaba con el costo ACTUAL del catálogo
-- (`productos.costo`), no con el que tenía el producto cuando se vendió. Eso
-- hacía que el margen de un período pasado cambiara al actualizar un costo, y
-- en los combos era peor todavía: su costo es una foto congelada de sus
-- componentes que nada propaga cuando el proveedor sube un precio.
--
-- `sale_items.unit_cost` congela el costo en el momento de la venta:
--   * simple / ingredient / prepared -> `productos.costo`
--   * combo                          -> suma de `costo x cantidad` de sus
--                                       productos incluidos, resuelta en el RPC
--
-- Nulo significa "costo desconocido" y la línea se excluye de la utilidad,
-- nunca se asume 0 (misma regla que ya regía en los reportes).

alter table public.sale_items
  add column if not exists unit_cost numeric(14,2);

comment on column public.sale_items.unit_cost is
  'Costo unitario del producto en el momento de la venta (ADR 0019). Nulo = costo desconocido: la línea se excluye de la utilidad, no se asume 0. En los combos es la suma del costo de los productos incluidos.';

-- Backfill de las ventas anteriores a esta migración: se usa el costo actual
-- del catálogo, que es exactamente lo que los reportes ya venían mostrando. No
-- mejora esas filas, pero tampoco las empeora ni hace desaparecer el histórico.
update public.sale_items si
set unit_cost = p.costo
...
-- =====================================================
-- create_sale_atomic: captura el costo de cada línea
-- =====================================================
-- Cuerpo idéntico a 20260803200100_combo_support.sql salvo la resolución del
-- costo y su persistencia.
...
declare
  ...
  -- Costo unitario capturado en la venta (ADR 0019).
  v_unit_cost numeric;
  v_option_id uuid;
...
begin
  ...
  loop
    ...
    select id, nombre, sku, tipo, is_active, precio_venta, iva_tasa, participa_fidelizacion, costo
    into v_product
    ...
    -- Costo unitario capturado en la venta (ADR 0019). Se congela aquí para que
    -- el margen histórico no cambie cuando cambie el costo del catálogo.
    --
    -- El combo se costea desde sus productos incluidos porque esa lista ES su
    -- lista completa de materiales. Un preparado no: sus componentes son el vaso
    -- y el sachet, pero la proteína base sale del tarro y no se mide por batido
    -- (ADR 0017), así que sumarlos daría un costo parcial disfrazado de total.
    if v_product.tipo = 'combo' then
      -- Si a cualquier producto incluido le falta costo, la línea queda sin
      -- costo: nunca se reporta un costo incompleto como si fuera completo.
      select case
               when bool_or(comp.costo is null) then null
               else sum(pc.cantidad * comp.costo)
             end
      into v_unit_cost
      from public.product_components pc
      join public.productos comp on comp.id = pc.componente_id
      where pc.producto_id = v_product.id
        and pc.tienda_id = p_tienda_id;
    else
      v_unit_cost := v_product.costo;
    end if;
...
-- =====================================================
-- get_reinvestment_fund_totals: COGS desde el costo capturado
-- =====================================================
-- Antes unía con `productos` para leer el costo actual, con el mismo sesgo que
-- los reportes. Ahora lee `sale_items.unit_cost`, así que el fondo de
-- reinversión deja de moverse cuando se actualiza un costo del catálogo.
...
as $$
  with cogs as (
    select s.created_at, si.quantity * si.unit_cost as costo
    from sale_items si
