-- Sube el umbral de descuento que un cajero puede aplicar sin aprobación de
-- admin, de 10% a 50% del subtotal. Decisión de negocio del usuario
-- (2026-06-23): el 10% resultaba demasiado restrictivo para la operación
-- diaria. El admin sigue sin tener tope (igual que antes); por encima del
-- 50% el cajero sigue bloqueado, igual que antes pero con el umbral nuevo.
--
-- Cuerpo idéntico a la versión vigente (20260617_001_fix_cash_change_persistence.sql)
-- salvo las dos comparaciones `0.10` -> `0.50` y el mensaje de error.
...
begin
  ...
  if v_role = 'cajero' and v_discount_total > round(v_subtotal * 0.50, 2) then
    raise exception 'Descuentos mayores al 50%% requieren aprobación de admin';
  end if;

  if v_role = 'admin' and v_discount_total > round(v_subtotal * 0.50, 2) then
