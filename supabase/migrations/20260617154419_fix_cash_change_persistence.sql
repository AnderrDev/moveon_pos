-- Bug: cuando un pago en efectivo incluye vuelto (el cliente paga con un
-- billete mayor al total), create_sale_atomic persistía en `payments.amount`
-- el monto BRUTO recibido (ej. 50000 para una venta de 26000), no el monto
-- neto aplicado a la venta. El vuelto se calculaba y mostraba correctamente
-- en la UI, pero nunca se descontaba antes de guardar el pago.
--
-- Esto infla `payments.amount` (y por lo tanto cualquier suma que dependa de
-- él: cierre de caja `close_cash_session_atomic`, reportes por método de
-- pago) por la suma de todos los vueltos en efectivo del período.
--
-- Caso real que originó este fix: venta V-000021 del 2026-06-16, total
-- $26.000 pagada con $50.000 en efectivo -> payments.amount quedó en 50000
-- en vez de 26000, inflando el cierre de caja de esa sesión en $24.000.
--
-- Fix: antes de insertar cada pago, se descuenta el vuelto (total pagado -
-- total de la venta) de los pagos en efectivo, en orden, hasta agotarlo.
-- Por construcción (ver validación `El cambio solo puede generarse desde
-- pagos en efectivo` más arriba en la función) el vuelto nunca puede superar
-- la suma de los pagos en efectivo, así que la resta nunca deja un pago en
-- negativo.
