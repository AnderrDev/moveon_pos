-- =====================================================
-- Opciones de producto en la venta (ADR 0017 / PLAN-71)
-- =====================================================
-- Un producto preparado puede ofrecer opciones excluyentes al momento de
-- venderlo (para los batidos: el tipo de proteína). Cada opción puede sumar un
-- recargo al precio y consumir un producto de inventario distinto.
--
-- Reglas (ADR 0017):
--   * El precio efectivo lo calcula el SERVIDOR: precio_venta + precio_extra.
--     Angular nunca decide el precio (igual que antes de este cambio).
--   * `sale_items` guarda snapshots (`option_nombre`, `option_extra`) para que
--     una venta pasada no cambie si luego se renombra la opción o se sube el
--     recargo.
--   * El descuento de inventario lo hace el trigger de componentes, con la
--     misma política vigente: advertir, no bloquear (el stock puede quedar
--     negativo).
-- =====================================================
...
create table if not exists public.product_options (
  ...
  grupo               text          not null default 'Proteína',
  nombre              text          not null,
  precio_extra        numeric(12,2) not null default 0 check (precio_extra >= 0),
  componente_id       uuid          references public.productos(id) on delete set null,
  componente_cantidad numeric(10,3) not null default 0 check (componente_cantidad >= 0),
  es_default          boolean       not null default false,
  orden               integer       not null default 0,
  is_active           boolean       not null default true,
  created_at          timestamptz   not null default now(),
  updated_at          timestamptz   not null default now(),
  constraint product_options_unique unique (producto_id, grupo, nombre)
);

create index if not exists ix_product_options_producto
  on public.product_options (producto_id, tienda_id)
  where is_active;

-- Una sola opción por defecto por grupo.
create unique index if not exists ux_product_options_default
  on public.product_options (producto_id, grupo)
  where es_default and is_active;
...
do $$ begin
  if not exists (
    ...
    where schemaname = 'public' and tablename = 'product_options' and policyname = 'po_read'
  ) then
      ...
      for select using (
        exists (
          ...
          where user_id   = auth.uid()
            and tienda_id = product_options.tienda_id
            and is_active = true
        )
      );
  end if;
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'product_options' and policyname = 'po_admin_write'
  ) then
    create policy "po_admin_write" on public.product_options
      for all using (
        exists (
          select 1 from public.user_tiendas
          where user_id   = auth.uid()
...
comment on table public.product_options is
  'Opciones excluyentes que el cajero elige al vender un producto (ej. tipo de proteína del batido). `precio_extra` se suma al precio de venta; `componente_id` se descuenta del inventario al vender. ADR 0017.';
comment on column public.product_options.grupo is
  'Etiqueta del selector en el POS (ej. "Proteína"). Las opciones del mismo grupo son excluyentes entre sí.';
comment on column public.product_options.componente_cantidad is
  'Unidades del componente consumidas por cada unidad vendida del producto. 0 = la opción no descuenta inventario.';

-- ===== Snapshot de la opción en la línea de venta =====
alter table public.sale_items
  add column if not exists option_id     uuid references public.product_options(id) on delete set null,
  add column if not exists option_nombre text,
  add column if not exists option_extra  numeric(12,2) not null default 0;

comment on column public.sale_items.option_nombre is
  'Snapshot del nombre de la opción elegida al momento de la venta (ADR 0017).';
comment on column public.sale_items.option_extra is
  'Snapshot del recargo unitario de la opción. Ya está incluido en `unit_price`; se guarda aparte solo como desglose.';

-- =====================================================
-- create_sale_atomic: precio efectivo = precio_venta + precio_extra
-- =====================================================
-- Cuerpo idéntico a 20260802125014_allow_full_discount.sql salvo la resolución
-- de la opción, el uso de `v_unit_price` en vez de `v_product.precio_venta` y
-- las tres columnas nuevas de `sale_items`.
...
-- =====================================================
-- Consumo de inventario: componentes fijos + componente de la opción
-- =====================================================
-- Cuerpo idéntico a 20260615000500_product_components.sql más el bloque de la
-- opción elegida. Política sin cambios: el stock puede quedar negativo
-- (advertir, no bloquear) — un batido no se cae por falta de sachet.
